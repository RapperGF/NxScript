...
class TestSuite {
	...
	static function testBugFixes() {
		...
		var calls = 0;
		i.register("getVal", 0, function(_) {
			calls++;
			return VNumber(10);
		});
	...
	static function testImports() {
		sec("Imports");
		var i = new Interpreter();
		ok(i.runDynamic('import "haxe.ds.StringMap"\nvar m=new StringMap()\nm.set("k",10)\nm.get("k")', "t.nx") == 10, 'import "haxe.ds.StringMap"');
		ok(i.runDynamic('import haxe.ds.StringMap\nvar m=new StringMap()\nm.set("k",42)\nm.get("k")', "t.nx") == 42, "import bare identifier");
		ok(i.runDynamic('function add(a:Int,b:Int):Int{return a+b;}\nadd(8,9);', "t.nx") == 17, "Haxe-style :ReturnType syntax");
	}
	...
	static function testNewFeatures() {
		...
		var i = new Interpreter();
		ok(i.runDynamic('var r="none"\ntry{throw "oops"\nr="bad"}catch(e){r=e}\nr') == "oops", "try/catch catches thrown value");
		ok(i.runDynamic('var r=0\ntry{r=1\nr=2}catch(e){r=-1}\nr') == 2, "try runs normally when no throw");
	...
	// ══════════════════════════════════════════════════════════════════════
	// 22. SWITCH (alias for match)
	// ══════════════════════════════════════════════════════════════════════
	static function testSwitchCases() {
		sec("switch case matching");
		var i = new Interpreter();
		ok(i.runDynamic('var x=2\nswitch x{case 1=>"one"\ncase 2=>"two"\ndefault=>"other"}') == "two", "value");
		ok(i.runDynamic('switch 99{case 1=>"one"\ndefault=>"other"}') == "other", "default");
		ok(i.runDynamic('var s=85\nswitch s{case 90...100=>"A"\ncase 80...89=>"B"\ndefault=>"F"}') == "B", "range");
		ok(i.runDynamic('switch 7{case 1=>"one"\ncase n=>n*10}') == 70, "bind case");
		ok(i.runDynamic('switch -5{case -5=>"neg"\ndefault=>"no"}') == "neg", "negative literal");
		ok(i.runDynamic('switch null{case null=>"nil"\ndefault=>"no"}') == "nil", "null literal");
		ok(i.runDynamic('switch true{case true=>"yes"\ndefault=>"no"}') == "yes", "bool literal");
		ok(i.runDynamic('var r=0\nswitch 3{case 1=>{r=100}\ncase 3=>{var t=300\nr=t+33}\ndefault=>{r=-1}}\nr') == 333, "block body");
		ok(i.runDynamic('var c="attack"\nswitch c{case "attack"=>"go"\ndefault=>"no"}') == "go", "string values");
		ok(i.runDynamic('switch [10,20,30]{case [a,b]=>a+b\ncase [a,b,c]=>a+b+c\ndefault=>0}') == 60, "array destructure");
		ok(i.runDynamic('enum Color{Red,Green,Blue}\nvar c=Color["Green"]\nswitch c{case Red=>"r"\ncase Green=>"g"\ncase Blue=>"b"}') == "g", "enum variant");
		ok(i.runDynamic('enum Reply{Ok(msg),Err(code)}\nvar r=Reply["Ok"]("hi")\nswitch r{case Ok(msg)=>msg\ndefault=>"no"}') == "hi", "enum payload");
		ok(i.runDynamic('switch 2{case 1=>"one"\ncase 2=>{var nested="a"\nvar result="no"\nswitch nested{case "a"=>result="nested"\ndefault=>result="no"}\nresult}\ndefault=>"no"}') == "nested",
			"nested switch");
		ok(i.runDynamic('switch 1{case 1=>"one"}') == "one", "no default");
	}
